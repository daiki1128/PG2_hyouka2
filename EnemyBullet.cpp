...
EnemyBullet::EnemyBullet(const Vector2& startPos, const Vector2& velocity) {
	pos_ = startPos;
	velocity_ = velocity;
	width_ = 64.0f;
	height_ = 64.0f;
...
void EnemyBullet::Draw(const Matrix3x3& worldToScreen) const {
	if (!isAlive_) return;
...
const Vector2& EnemyBullet::GetPos() const {
	return pos_;
}